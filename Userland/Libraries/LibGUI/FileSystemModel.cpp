...
void FileSystemModel::Node::traverse_if_needed()
{
    ...
    if (!is_directory()) {
        if (!m_model.m_file_watcher->is_watching(full_path)) {
            ...
            if (result.is_error()) {
                dbgln("Couldn't watch FILE '{}': {}", full_path, result.error());
            } else if (!result.value()) {
                dbgln("Couldn't watch FILE '{}', probably already watching", full_path);
            }
        }
        return;
    }
...
void FileSystemModel::handle_file_event(Core::FileWatcherEvent const& event)
{
    ...
    case Core::FileWatcherEvent::Type::MetadataModified: {
        ...
        int rc;
        if (&child.value() == m_root)
            rc = stat(child->full_path().characters(), &st);
        else
            rc = lstat(child->full_path().characters(), &st);
        if (rc < 0) {
            dbgln("stat: {}", Error::from_errno(errno).string_literal());
            break;
        }

        auto& mutable_child = const_cast<Node&>(child.value());
        mutable_child.size = st.st_size;
        mutable_child.mode = st.st_mode;
        mutable_child.uid = st.st_uid;
