...
Gfx::IntRect WindowSwitcherOverlay::item_rect(size_t index) const
{
    return {
        padding(),
        padding() + static_cast<int>(index) * item_height(),
        rect().width() - padding() * 2,
        item_height()
    };
}

void WindowSwitcherOverlay::render_overlay_bitmap(Gfx::Painter& painter)
{
    dbgln("WindowSwitcherOverlay: rendering overlay bitmap!");
    ...
    for (size_t index = 0; index < m_windows.size(); ++index) {
        ...
        Gfx::IntRect const thumbnail_rect = { item_rect.location().translated(0, 5), { thumbnail_width(), thumbnail_height() } };
        ...
        Gfx::IntRect const icon_rect = { thumbnail_rect.bottom_right().translated(-window.icon().width() - 1, -window.icon().height() - 1), { window.icon().width(), window.icon().height() } };
...
void WindowSwitcherOverlay::refresh()
{
    dbgln("WindowSwitcherOverlay: refreshing!");
    auto& wm = WindowManager::the();
        ...
        window_stack.for_each_window_of_type_from_front_to_back(
            WindowType::Normal, [&](Window& window) {
                if (window.is_frameless() || window.is_modal())
                    return IterationDecision::Continue;
                ++window_count;
                longest_title_width = max(longest_title_width, static_cast<int>(wm.font().width(window.computed_title())));
                if (selected_window == &window)
                    m_selected_index = static_cast<int>(m_windows.size());
    ...
    int const space_for_window_details = 200;

    Gfx::IntRect new_rect;
    new_rect.set_width(thumbnail_width() + longest_title_width + space_for_window_details + padding() * 2 + item_padding() * 2);
    new_rect.set_height(window_count * item_height() + padding() * 2);
    new_rect.center_within(Screen::main().rect());
    set_rect(new_rect);

    invalidate_content();
}

Window* WindowSwitcherOverlay::selected_window()
{
...
void WindowSwitcherOverlay::event(Core::Event& event)
{
    dbgln("WindowSwitcherOverlay: handling event!");
    ...
    for (size_t i = 0; i < m_windows.size(); ++i) {
        ...
        if (item_rect.contains(mouse_event.position())) {
            new_hovered_index = static_cast<int>(i);
            break;
        }
    }
...
void WindowSwitcherOverlay::on_key_event(KeyEvent const& event)
{
...
void WindowSwitcherOverlay::select_window(Window& window)
{
    for (size_t i = 0; i < m_windows.size(); ++i) {
...
void WindowSwitcherOverlay::select_window_at_index(int index)
{
...
void WindowSwitcherOverlay::clear_hovered_index()
{
